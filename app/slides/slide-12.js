function createSlide(pres, theme) {
  ...
  topStats.forEach((s, i) => {
    ...
    slide.addText(s.val, { x: x, y: 1.18, w: 2.15, h: 0.6, fontSize: 30, bold: true, color: theme.accent, align: "center", fontFace: "Segoe UI" });
    slide.addText(s.label, { x: x + 0.1, y: 1.75, w: 1.95, h: 0.25, fontSize: 10, bold: true, color: theme.primary, align: "center", fontFace: "Segoe UI" });
    slide.addText(s.sub, { x: x + 0.1, y: 1.98, w: 1.95, h: 0.2, fontSize: 9, color: theme.secondary, align: "center", fontFace: "Segoe UI" });
  });
  ...
  bars.forEach((b, i) => {
    const y = 2.85 + i * 0.52;
    slide.addText(b.label, { x: 0.5, y: y, w: 0.5, h: 0.42, fontSize: 11, bold: true, color: theme.secondary, valign: "middle", fontFace: "Segoe UI" });
    ...
    slide.addText((b.val / 1000).toFixed(1) + "s", { x: 1.1 + barW, y: y, w: 0.8, h: 0.42, fontSize: 11, color: theme.primary, valign: "middle", fontFace: "Segoe UI" });
  });

  // VRAM note
  slide.addShape(pres.shapes.RECTANGLE, { x: 7.0, y: 2.85, w: 2.5, h: 1.3, fill: { color: theme.light }, rectRadius: 0.08 });
  slide.addText("VRAM stable", { x: 7.15, y: 2.95, w: 2.2, h: 0.3, fontSize: 11, bold: true, color: "10b981", fontFace: "Segoe UI" });
  ...
  slide.addText("12", { x: 9.3, y: 5.1, w: 0.5, h: 0.35, fontSize: 10, color: theme.secondary, fontFace: "Segoe UI" });
}
